...
class Features:
    ...
    def get_variance_mean_max_min(self):
        ...
        for name in POINT_OF_INTREST:
            ...
            positions_x = np.array(
                [body_coordinate[sitting_body][name].x if name in body_coordinate[sitting_body].keys() else 0 for
                 body_coordinate in self.all_normal_coordinates])
            positions_y = np.array(
                [body_coordinate[sitting_body][name].y if name in body_coordinate[sitting_body].keys() else 0 for
                 body_coordinate in self.all_normal_coordinates])
            positions_z = np.array(
                [body_coordinate[sitting_body][name].z if name in body_coordinate[sitting_body].keys() else 0 for
                 body_coordinate in self.all_normal_coordinates])

            if np.count_nonzero(positions_x) > 0 and np.count_nonzero(positions_y) > 0 and np.count_nonzero(
                    positions_z) > 0:
    ...
    def get_degree(self, name1, name2, name3):
        ...
        try:
            ...
            points_a = np.array(
                [body_coordinate[sitting_body][name1] for body_coordinate in self.all_normal_coordinates])
            points_b = np.array(
                [body_coordinate[sitting_body][name2] for body_coordinate in self.all_normal_coordinates])
            points_c = np.array(
                [body_coordinate[sitting_body][name3] for body_coordinate in self.all_normal_coordinates])
